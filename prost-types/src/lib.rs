...
impl From<Timestamp> for time::SystemTime {
    fn from(mut timestamp: Timestamp) -> time::SystemTime {
        timestamp.normalize();
        let system_time = if timestamp.seconds >= 0 {
            time::UNIX_EPOCH + time::Duration::from_secs(timestamp.seconds as u64)
        } else {
            time::UNIX_EPOCH - time::Duration::from_secs((-timestamp.seconds) as u64)
        };
